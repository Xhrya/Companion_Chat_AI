...
const AiAssist = ({ props, activeChat }) => {
  ...
  const [triggerAssist, resultsAssist] = usePostAiAssistMutation();
  ...
  useEffect(() => {
    if (debouncedValue) {
      ...
      //once we have form value, then we make trigger call
    }
  }, [debouncedValue]); /* tslint:disable:no-unused-variable */
  ...
  useEffect(() => {
    if (resultsAssist.data?.text) {
      setAppendText(resultsAssist.data?.text);
    }
  }, [resultsAssist]);
  return (
    <MessageFormUI
